...
const BuyerSaveProperty = () => {
  ...
  useEffect(() => {
    const fetchSavedProperties = async () => {
      const properties = await getSavedPropertiesController.getSavedProperties()
      ...
      {savedProperties.map((savedProperty) => (
        ...
        <img className="w-full" src={savedProperty.image} alt="img1"/>
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">{savedProperty.address}</div>
              <p className="text-gray-700 text-base">{savedProperty.description}</p>
          </div>
        </div>
      ))}
    </div>
